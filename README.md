...
the 8 characters are the first 8 hex digits of the HMAC-SHA1 hash of the
full path (e.g., `assets/img/thumbnail-100/0/img/foobar.png`).
...
assets/img/0/1/foo.png                     => t:8194d532
```

Note: URLs generated by the `url()` method always use the default CRC32
format for backward compatibility. If you need HMAC tokens in your URLs,
you must manually construct them using the same algorithm.
